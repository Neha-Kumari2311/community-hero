  ...
  try {
    ...
    const users = await User.create([
      ...
      // Karnataka Users
      {
        ...
        region: 'Karnataka',
        points: 180,
        badges: ['First Report', 'Verifier', 'certificate_bronze'],
        issuesReported: 9,
        issuesResolved: 4,
      },
      {
      ...
      {
        name: 'BBMP Admin',
        email: 'admin@bbmp.gov',
        password,
        role: 'admin',
        region: 'Karnataka',
        department: 'Public Works - BBMP',
        points: 0,
        badges: ['Admin'],
        issuesReported: 0,
        issuesResolved: 6,
    ...
    const issues = await Issue.create([
      ...
      {
        title: 'Massive pothole on Outer Ring Road near Marathahalli',
        description: 'A dangerous pothole has formed on Outer Ring Road near Marathahalli bridge. It is about 2.5 feet wide and 6 inches deep. Multiple two-wheeler accidents have happened here in the last 3 days. The pothole fills with water during rain making it invisible.',
        category: 'pothole',
        severity: 'critical',
        status: 'verified',
        region: 'Karnataka',
        images: ['https://images.unsplash.com/photo-1560782202-154b39d57ef2?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'],
        location: {
          address: 'Outer Ring Road, Near Marathahalli Bridge, Bangalore',
          lat: 12.9568,
          lng: 77.7010,
        },
        ...
        aiAnalysis: {
          category: 'pothole',
          severity: 'critical',
          description: 'Large pothole on IT corridor main road causing accidents',
          suggested_department: 'BBMP - Road Infrastructure',
          estimated_impact: 'Affects 100,000+ daily commuters on ORR, high accident risk',
          priority_score: 9,
          resolution_steps: ['Place barricades immediately', 'Remove damaged asphalt', 'Fill with hot-mix', 'Compact and level', 'Allow 48hrs curing'],
          estimated_resolution_time: '2-3 days',
        },
        upvotes: [users[6]._id, users[0]._id, users[1]._id],
        verifiedBy: [users[6]._id, users[0]._id],
        comments: [
          { user: users[6]._id, text: 'My colleague broke his leg falling here last week! BBMP please fix this!', createdAt: new Date(Date.now() - 86400000) },
          { user: users[5]._id, text: 'This has been here for 2 months. Completely ignored by authorities.', createdAt: new Date(Date.now() - 43200000) },
        ],
      },
      {
        title: 'Severe waterlogging at Silk Board Junction underpass',
        description: 'The underpass at Silk Board Junction floods every time it rains even slightly. Yesterday 2 hours of rain caused 3 feet waterlogging. Vehicles were stuck for hours. Drainage system completely blocked.',
        category: 'drainage',
        severity: 'critical',
        status: 'reported',
        region: 'Karnataka',
        images: ['https://images.unsplash.com/photo-1636333855543-ceaaf263747d?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'],
        location: {
          address: 'Silk Board Junction Underpass, Bangalore',
          lat: 12.9177,
          lng: 77.6238,
        },
        ...
        aiAnalysis: {
          category: 'drainage',
          severity: 'critical',
          description: 'Chronic waterlogging at major traffic junction due to blocked drains',
          suggested_department: 'BBMP - Storm Water Drain Division',
          estimated_impact: 'Paralyzes traffic for 50,000+ vehicles daily during monsoon',
          priority_score: 10,
          resolution_steps: ['Emergency pump deployment', 'Clear blocked drain outlets', 'Desilt underground pipes', 'Install high-capacity pumps', 'Raise road level at low points'],
          estimated_resolution_time: '5-7 days (emergency) + 30 days (permanent fix)',
        },
        upvotes: [users[5]._id, users[0]._id, users[1]._id, users[2]._id],
        verifiedBy: [users[5]._id],
        comments: [
          { user: users[5]._id, text: 'This happens EVERY SINGLE RAIN. When will BBMP permanently fix this?', createdAt: new Date(Date.now() - 21600000) },
        ],
      },
      {
        title: 'Garbage not collected for 5 days in Koramangala 4th Block',
        description: 'BBMP garbage truck has not come to Koramangala 4th Block for the last 5 days. All community bins are overflowing. Stray dogs tearing garbage bags. Terrible smell and mosquito breeding. Multiple calls to BBMP helpline but no response.',
        category: 'waste_management',
        severity: 'high',
        status: 'reported',
        region: 'Karnataka',
        images: ['https://images.unsplash.com/photo-1530587191325-3db32d826c18?w=800&h=600&fit=crop'],
        location: {
          address: 'Koramangala 4th Block, 80 Feet Road, Bangalore',
          lat: 12.9352,
          lng: 77.6245,
        },
        reportedBy: users[5]._id,
        aiAnalysis: {
          category: 'waste_management',
          severity: 'high',
          description: 'Garbage collection failure causing hygiene crisis in residential area',
          suggested_department: 'BBMP - Solid Waste Management',
          estimated_impact: 'Health hazard for 3000+ residents, dengue risk',
          priority_score: 8,
          resolution_steps: ['Deploy backup garbage truck immediately', 'Clear overflowing bins', 'Spray anti-mosquito solution', 'Penalize contractor for missed pickups', 'Set up complaint tracking'],
          estimated_resolution_time: '1 day (emergency pickup)',
        },
        upvotes: [users[6]._id, users[1]._id],
        verifiedBy: [users[6]._id],
        comments: [
          { user: users[6]._id, text: 'Same issue every month! The contractor is useless. BBMP needs to change them.', createdAt: new Date(Date.now() - 172800000) },
        ],
      },
      {
        title: 'Broken streetlights on entire stretch of HSR Layout Sector 2',
        description: 'All streetlights on the main road of HSR Layout Sector 2 (from 27th Main to BDA Complex) have been off for over a week. The 1.5km stretch is completely dark at night. Women feel unsafe walking. Two chain-snatching incidents reported.',
        category: 'streetlight',
        severity: 'high',
        status: 'in_progress',
        region: 'Karnataka',
        images: ['https://images.unsplash.com/photo-1547769725-b95b3ae93d42?q=80&w=654&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'],
        location: {
          address: 'HSR Layout Sector 2, 27th Main Road, Bangalore',
          lat: 12.9116,
          lng: 77.6389,
        },
        reportedBy: users[6]._id,
        aiAnalysis: {
          category: 'streetlight',
          severity: 'high',
          description: 'Multiple streetlights non-functional creating safety hazard',
          suggested_department: 'BESCOM / BBMP Electrical Wing',
          estimated_impact: 'Safety risk for pedestrians, increased crime incidents',
          priority_score: 8,
          resolution_steps: ['Inspect transformer and wiring', 'Replace damaged cables', 'Install new LED panels', 'Test all connections', 'Set up periodic maintenance schedule'],
          estimated_resolution_time: '3-5 days',
        },
        upvotes: [users[5]._id, users[0]._id],
        verifiedBy: [users[5]._id, users[0]._id, users[1]._id],
        comments: [
          { user: users[5]._id, text: 'Two chain snatching incidents happened because of darkness. Please fix urgently!', createdAt: new Date(Date.now() - 86400000) },
        ],
        assignedTo: 'BESCOM - HSR Division',
      },
      {
        title: 'Dangerous open manhole near Indiranagar Metro Station',
        description: 'A manhole cover is missing on 100 Feet Road near Indiranagar Metro Station Exit 2. It is right on the footpath where hundreds of people walk daily. At night it is invisible. A child almost fell in yesterday.',
        ...
        images: ['https://images.unsplash.com/photo-1653420724129-794516c863b8?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'],
        location: {
          address: '100 Feet Road, Near Indiranagar Metro Station, Bangalore',
          lat: 12.9784,
          lng: 77.6408,
        },
        reportedBy: users[5]._id,
        aiAnalysis: {
          category: 'safety_hazard',
          severity: 'critical',
          description: 'Open manhole on busy pedestrian path - life-threatening danger',
          suggested_department: 'BWSSB / BBMP Maintenance',
          estimated_impact: 'Risk of fatal fall for pedestrians, especially children and elderly',
          priority_score: 10,
          resolution_steps: ['Emergency barricade placement', 'Deploy traffic cones', 'Source replacement cover', 'Install heavy-duty anti-theft cover', 'File FIR for stolen cover'],
          estimated_resolution_time: 'Immediate (barricade) + 1 day (permanent cover)',
        },
        upvotes: [users[6]._id, users[0]._id, users[1]._id, users[2]._id, users[3]._id],
        verifiedBy: [users[6]._id, users[0]._id, users[1]._id],
        comments: [
          { user: users[6]._id, text: 'I put some bricks around it as warning. But this needs immediate BBMP action!', createdAt: new Date(Date.now() - 43200000) },
          { user: users[5]._id, text: 'Reported to BBMP Sahaaya app 3 days ago. No response at all.', createdAt: new Date(Date.now() - 21600000) },
        ],
      },
      {
        title: 'Illegal construction debris dumped at Bellandur Lake bund',
        description: 'Someone has dumped truckloads of construction waste (concrete, bricks, tiles) on the Bellandur Lake bund road. This is encroachment on lake land and blocking the walking path. This happened overnight - likely illegal dumping by a builder.',
        category: 'pollution',
        severity: 'high',
        status: 'reported',
        region: 'Karnataka',
        images: ['https://images.unsplash.com/photo-1611273426858-450d8e3c9fce?w=800&h=600&fit=crop'],
        location: {
          address: 'Bellandur Lake Bund Road, Bangalore',
          lat: 12.9260,
          lng: 77.6762,
        },
        reportedBy: users[6]._id,
        aiAnalysis: {
          category: 'pollution',
          severity: 'high',
          description: 'Illegal construction debris dumping on lake land',
          suggested_department: 'BDA / Karnataka State Pollution Control Board',
          estimated_impact: 'Lake ecosystem damage, public path blocked, illegal land encroachment',
          priority_score: 7,
          resolution_steps: ['Check CCTV for dumping truck details', 'File complaint with BDA', 'Deploy trucks to remove debris', 'Install No Dumping signage', 'Increase night patrolling'],
          estimated_resolution_time: '5-7 days',
        },
        upvotes: [users[5]._id, users[0]._id],
        verifiedBy: [],
        comments: [
          { user: users[5]._id, text: 'This is destroying our lake! We need to identify and fine the builder responsible.', createdAt: new Date(Date.now() - 86400000) },
        ],
      },
    ]);

    console.log(`📋 Created ${issues.length} issues (6 Delhi + 6 Bangalore)`);
